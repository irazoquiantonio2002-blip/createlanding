  ...
  return (
    <div>
      <div className="flex items-center gap-4">
        <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-xl border border-dashed border-gris-300 bg-gris-50">
          {url ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={url} alt="Logo del negocio" className="h-full w-full object-contain p-1.5" />
          ) : (
            <span className="text-xs text-gris-400">Sin logo</span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-lg border border-gris-300 bg-white px-3 py-2 text-sm font-medium text-gris-700 transition hover:bg-gris-50 disabled:opacity-50"
            >
            ...
            {url && (
              <button
                ...
                className="rounded-lg px-3 py-2 text-sm text-gris-500 transition hover:text-rose-600"
              >
                Quitar
              </button>
            )}
        ...
        <ul className="mb-3 grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6">
          {imagenes.map((img, i) => (
            <li key={img.path} className="group relative">
              <div className="aspect-square overflow-hidden rounded-lg border border-gris-200 bg-gris-50">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={img.url} alt={img.nombre} className="h-full w-full object-cover" />
              </div>
              <span className="absolute top-1 left-1 rounded bg-gris-950/70 px-1.5 py-0.5 text-[10px] font-medium text-white">
                {i + 1}
              </span>
              <button
                type="button"
                ...
                className="absolute top-1 right-1 grid h-6 w-6 place-items-center rounded-full bg-gris-950/70 text-sm text-white opacity-0 transition group-hover:opacity-100 hover:bg-rose-600 focus:opacity-100"
              >
                ×
              </button>
            </li>
